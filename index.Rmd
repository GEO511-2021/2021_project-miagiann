...
map <- leaflet() %>%
  ...
  addCircleMarkers(data = tree_pop_data, lng = tree_pop_data$longitude, lat = tree_pop_data$latitude, radius = 0.0001, color = ~factpal(estimate), clusterOptions = markerClusterOptions())
                   
map
```

```{r, echo = FALSE}
file.remove (tree_census_file)
```
# Conclusions
